...
class JoystickModule extends GameWindow {
    ...
    protected onInit(){
        ...
        this._touchArea = this.skin.m_joystick_touch;
        this._center = this.skin.m_joystick_center;
    ...
    private OnTouchUp(evt: egret.TouchEvent): void {
        if(this.touchId != -1 && evt.touchPointID == this.touchId) {
            ...
            JoystickModule.multX = 0;
            JoystickModule.multY = 0;
            JoystickModule.offset = 0;
    ...
    private OnTouchMove(evt: egret.TouchEvent): void {
        if(this.touchId != -1 && evt.touchPointID == this.touchId) {
            ...
            JoystickModule.multX = Math.cos(rad) * this.speed;
            JoystickModule.multY = Math.sin(rad) * this.speed;
            JoystickModule.offset = Math.pow(Math.exp(offsetX) + Math.exp(offsetY),-2);
